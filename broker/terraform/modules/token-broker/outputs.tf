...
output "execute_api_endpoint" {
  description = <<-EOT
    The generated execute-api URL, always. In production this MUST return 404 once
...
output "target_domain_name" {
  description = <<-EOT
    ...
    single request 404s at the origin. The smoke workflow then reports
    `GET /healthz -> 404`, which matches nothing in either runbook's error ladder — while
    the leaf README's own "execute-api MUST return 404" check makes the failure look like
