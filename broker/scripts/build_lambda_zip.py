...
`httpx`. Nothing else.
  ...
  * `app/main.py` is EXCLUDED BY NAME. brimyr's broker does not ship one (chargate's
    was a FastAPI dev shell; LocalStack covers that here). The exclusion is kept so
    that adding a dev-only shell later cannot silently drag FastAPI into the zip and
    turn a working deploy into a cold-start ImportError.
...
never matches, and every brimyr release opens a version-bump PR in magmamoose/infra
...
def changed_since(broker_root: Path, ref: str) -> bool:
    ...
    THE PUBLISH GATE. Brimyr releases are overwhelmingly CLI changes that touch
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--out", type=Path, default=Path("dist/brimyr-broker.zip"))
    parser.add_argument(
        "--platform",
        required=True,
